<script>
  ...
  import ModalCreate from "../../components/FloatingButtons/CreateNasServer.svelte"
  ...
  let columns = [
    ...
    { 
      ...
      formatter: (cell, row) => {
        return h('button', {
          className: 'btn',
          onClick: () => window.location.href = `/web-root-manager/nas-servers/${row.cells[0].data}/view-omv`
        }, 'VIEW');
      }
    },
  ]
...
<div class="row" style="background: #555;">
  ...
  <div class="col s10">
    ...
    <p style="color: #eee;">These records represent Network Attached Storage (NAS).</p>
    <ul class="tabs tabs-fixed-width">
      <li class="tab col s3"><a class="active" href="#test1">OMV</a></li>
      <li class="tab col s3"><a class="active" href="#test2">FreeNAS</a></li>
    </ul>
  </div>
</div>
